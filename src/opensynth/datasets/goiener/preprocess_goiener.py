...
def drop_dupes_and_replace_nulls(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_out = df_out.sort_values(
        by=["profile_id", "date", "settlement_period"], ascending=True
    )
    df_out = df_out.drop_duplicates(
        subset=["profile_id", "date", "settlement_period"], keep="last"
    )
...
def pack_smart_meter_data_into_arrays(df: pd.DataFrame) -> pd.DataFrame:
    ...
    groupby_cols = ["profile_id", "month_end", "month"]
...
def preprocess_pipeline(file_path: Path, out_path: Path):
    ...
    df.to_csv(f"{out_path}/goiener_data.csv", index=False)
...
def preprocess_goiener_data():
    ...
    SOURCE_DIR = "data/raw/goiener"
    OUT_DIR = "data/processed/goiener"
    preprocess_pipeline(
