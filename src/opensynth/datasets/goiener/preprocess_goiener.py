...
def encode_categorical_features(
    ...
    for col in feature_cols:
        ...
        if (
            pd.api.types.is_object_dtype(df_out[col])
            or pd.api.types.is_bool_dtype(df_out[col])
        ):
            cat = pd.Categorical(df_out[col].astype(str))
            df_out[col] = cat.codes.astype(int)
            encoders[col] = {"categories": list(cat.categories)}
        else:
            encoders[col] = {"type": "numeric_passthrough"}
...
def pack_smart_meter_data_into_arrays(
    ...
    Note: GoiEner dataset are all given in UTC. We should expect
    48-readings per day.
    ...
    if feature_cols:
        groupby_cols = groupby_cols + feature_cols
...
def preprocess_pipeline(
    ...
    # df, feature_encoders = encode_categorical_features(df, feature_cols)

    os.makedirs(out_path, exist_ok=True)

    # with open(f"{out_path}/feature_encoders.json", "w") as f:
    #     json.dump(feature_encoders, f, indent=2, default=str)
...
def preprocess_data(
    ...
    SOURCE_DIR = f"{data_dir}/raw/goiener"
    OUT_DIR = f"{data_dir}/processed/goiener"
    preprocess_pipeline(
