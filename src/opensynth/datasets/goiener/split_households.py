...
def split_historical_future_periods(
    ...
    Args:
        ...
        date_col (str, optional): Name of date column. Defaults to "timestamp".
...
def split_goiener_data(csv_filename: Path, sample_size: int = 2000):
    """
    Split GoiEner dataset 4 ways:
    ...
    logger.info(f"👀 Reading GoiEner data from: {csv_filename}")
    ...
    df_historical_train = df_history[df_history["profile_id"].isin(train_ids)]
    df_future_train = df_future[df_future["profile_id"].isin(train_ids)]
    df_historical_holdout = df_history[df_history["profile_id"].isin(holdout_ids)]
    df_future_holdout = df_future[df_future["profile_id"].isin(holdout_ids)]

    logger.info("📦 Saving train and holdout data")
    historical_path = Path("data/raw/goiener/historical")
    future_path = Path("data/raw/goiener/future")
